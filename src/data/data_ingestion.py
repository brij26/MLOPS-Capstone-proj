...
def load_params(params_path: str) -> dict:
    """Load a parameters from the yaml file"""
    try:
        with open(params_path, 'r') as file:
            params = yaml.safe_load(file)
        logging.debug("Parameters Retrives from %s", params_path)
        return params
...
def load_data(data_url: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.error(
            "Unexpected error occurred during loading the data: %s", e)
        raise
...
def main():
    try:
        ...
        # s3 = S3_operations('bucket_name', "aws_access_key", "aws_secret_key")
