...
def removing_punctuations(text):
    """Remove puctuations from the text"""
...
# Define your custom metris using this registry
...
def predict():
    ...
    features_df = pd.DataFrame(features.toarray(), columns=[
    ...
    # Increament prediction count metric
    PREDICTION_COUNT.labels(Prediction=str(prediction)).inc()

    # Measure latency
    REQUEST_LATENCY.labels(
        endpoint="/predict").observe(time.time() - start_time)
...
if __name__ == "__main__":
    ...
    # app.run(debug=True, host = "0.0.0.0", port = 5000) # Accesible from outside docker
